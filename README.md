...
- **Device-as-a-Service fleet** (since 22.09.2026, the default demo): a provider buys smartphones, tablets and laptops, rents them to business customers for 12 to 48 months, takes them back, wipes and grades them, repairs or refurbishes them, rents them a second time, and finally sells or recycles them. The console follows the cycle: Fleet, Returns (what comes back when), Warehouse compartments (new stock, returns intake, MDM release hold, wipe and grading, repair, refurbishment, second-life stock, sellable stock, swap buffer: each with its capacity, its dwell and a verdict, never mixed), a KPIs tab with the inventory and working-capital figures a fleet owner steers by, a Capacity plan tab (Little's law inverted: the fleet the owner wants by a date implies a return flow, the flow at today's dwell implies a stock per compartment, and the month that stock crosses the capacity is the lead time for a lease; two levers per compartment, the dwell it must reach and the extra places, neither picked for the planner; the milestones are the owner's and are edited through the API), and a Device TCO tab: what a device costs over its life and per month of service, per class and per model, and how much the second life gives back, for the finished lives (the whole-life number a rental price has to cover) and for the fleet to date. Quantities are measured from the order lines, the rental contracts, the repair and refurbishment invoices, the compartments and the sold serials; rates are placeholders with an owner, and the read aggregates in the database over every serial. Seeded at full size: **300,000 devices at customers, 100,000 in the warehouse**, 493,272 rental contracts, and a fleet that is visibly mid-scale-up — first rentals rising quarter on quarter, 28,000 devices still on order, two intake stations already over capacity. This is the default dataset; a database holding the other one is replaced on boot (`backend/app/seed_reset.py`), and `DAAS_SCALE=0.1` gives the same shape at a tenth of the size. A seeded snapshot only ages, so the console has a **Simulation tab** (since 24.09.2026): the day-to-day of the fleet fired at the running system, a delivery arriving, devices going out, rentals coming back, the return chain clearing, a partner stalling, a whole day at the fleet's own measured rates. Every event goes through the same service calls the console uses (`backend/app/services/cycle.py` on top of the asset service), so a move the state machine forbids is refused with its reason, and every answer says what moved between which compartments, what was refused and why, and which KPIs moved, before and after, measured again the moment the event ran. It writes to the demo database and says so; production refuses, the read-only guest cannot fire, and the way back is the boot's own rebuild.
...
- **Capacity plan (DaaS)** — `GET /capacity-plan`: per milestone and compartment the required throughput, the required stock at today's mean dwell against the capacity, the dwell it would have to reach and the extra places needed, plus the month each compartment breaks along the interpolated fleet path. `PUT /capacity-plan/milestones/{date}` sets a target fleet (PROCUREMENT), `PUT /capacity-plan/compartments/{code}/capacity` a station's capacity (WAREHOUSE); both return the recomputed plan. The dwell is derived and says so.
- **Simulation (DaaS, demo only)** — `GET /simulation/actions` lists the fourteen events with defaults filled from today's measured rates; `POST /simulation/actions/{id}` fires one (WAREHOUSE; ADMIN passes) and answers with the moves, the refusals with their reasons, the compartments and the capacity plan's breach before and after, and the KPIs a fleet event can move, measured again now; `POST /simulation/reset` rebuilds the dataset in a process of its own (ADMIN). Every write refuses under `SCM_ENV=prod` the way the seeders do. A batch touches at most 5,000 devices, never the fleet. A day of operation and a hold let time pass for real: `backend/app/services/timeshift.py` moves every date the fleet carries back by the days that passed (the one deliberate exception to "no direct writes", in that one place, with its reason), so waiting stock ages, contracts come due and a hold pushes the MDM-over-SLA KPI up, not down; the answer and every screen say how many days the dataset stands ahead of its seed. An event re-measures the fleet and warehouse KPIs only; the rest keep their measurement and say, on the KPIs tab and in the answer, how many simulated days ago it was taken.
...
- **TCO** — `GET /assets/{id}/tco` (per-asset waterfall), `GET /tco/portfolio` (TSCMC rollup), `GET /tco/by-class` — with a landed-type exclusion filter for tariff scenarios; the rollups aggregate in the database. `GET /tco/devices` is the device fleet's own view: cost per device and per month in service, per class and per model, finished lives and the fleet to date, every figure with the counts behind it and every rate marked as a placeholder with its owner.
...
- [x] **Device TCO for the rented fleet** ([`app/services/tco_device.py`](backend/app/services/tco_device.py), the Device TCO tab): the datacenter layers do not describe a rented phone, so the fleet has its own layer set: acquisition, inbound logistics, enrolment and configuration, software and management per month, support and damage, repair and refurbishment, warehousing and tied-up capital, and the resale credit at end of life. Quantities are measured (order lines, rental contracts, `service_event` invoices, compartment dwell, sold serials); rates are placeholders with an owner. Two populations: finished lives (the whole-life cost per device and per month in service, the number a rental price has to cover) and the whole fleet to date. Everything aggregates in the database from index scans; the per-asset rollups above were rewritten the same way after they held the only worker for nine minutes over a fleet with no datacenter layer.
