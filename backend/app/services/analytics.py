...
def _received_spend_rows(db: Session):
...
def spend_by_supplier(db: Session) -> list[dict]:
    totals: dict[str, dict] = defaultdict(lambda: {"units": 0, "spend": Decimal("0"), "name": None})
    for asset, oi, order, _product in _received_spend_rows(db):
...
def spend_by_product(db: Session) -> list[dict]:
    totals: dict[str, dict] = defaultdict(lambda: {"units": 0, "spend": Decimal("0"), "name": None, "category": None})
    for asset, oi, _order, product in _received_spend_rows(db):
...
def spend_summary(db: Session) -> dict:
    rows = _received_spend_rows(db)
    total = sum((_unit_price(oi) for _a, oi, _o, _p in rows), Decimal("0"))
    return {
        "total_units": len(rows),
        "total_spend": total,
        "by_supplier": spend_by_supplier(db),
